...
TcpServerSocket::TcpServerSocket(const string &localAddress,
                                 unsigned short localPort, int queueLen): Socket(SOCK_STREAM, IPPROTO_TCP) // CONTRUCTOR CO THAM SO DIA CHI + PORT
 // GIAI PHONG SOCKET KHI DOI TUONG BI HUY
{
#ifdef __linux__
    int reusePort = 1;
    setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, &reusePort, sizeof(reusePort));
...
// TAO SOCKET TCP SERVER ->DUNG SOCKET() VOI KIEU SOCK_STREAM
//GAN CONG DIA CHI CUC BO
//DUA SOCKET VAO TRANG THAI LANG NGHE
...
void TcpServerSocket::setListen(unsigned short localPort, int queueLength) //TAO SOCKET VA DUA VAO LISTEN
{
    if(sockDesc==-1)
    {
        // Make a new socket
...
bool TcpServerSocket::isListening() //KIEM TRA SERVER CO DANG LANG NGHE KHONG
{
#ifdef __linux__
    char val;
    if(sockDesc <= 0)
...
TcpSocket TcpServerSocket::accept(long timeout) // CHAP NHAN KET NOI CLIENT
{
    int newConnSD;
    if(timeout==0)
    {
...
void TcpServerSocket::listen(int queueLen) // DAT SOCKET SERVER VAO TRANG THAI CHO KET NOI
{
...
void TcpServerSocket::close() // GIAI PHONG SOCKET
{
#ifdef _WIN32
    if(sockDesc>0)
    {
