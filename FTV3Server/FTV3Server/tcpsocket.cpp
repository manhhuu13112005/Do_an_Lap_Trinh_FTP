...
{
    int rtn = ::recv(sockDesc, (raw_type *)buffer, bufferLen, 0);  // <-- bỏ -1
    if (rtn < 0) {
#ifdef _WIN32
    ...
    return rtn; // có thể bằng 0 nếu peer đóng
}

int TcpSocket::recv(char* buffer, int bufferLen)
{
    int rtn = ::recv(sockDesc, buffer, bufferLen, 0);  // <-- bỏ -1
    if (rtn < 0) {
#ifdef _WIN32
        if (WSAGetLastError() == WSAETIMEDOUT) return -2;
        else {
...
#endif
            buffer[total] = 0;  // peer đóng hoặc lỗi
            return total;
        }
