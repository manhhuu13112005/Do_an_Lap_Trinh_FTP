...
public:
     ...
     *   address and port
     */
    void connect(const string &remoteAddress, unsigned short remotePort);// ket noi toi server
     ...
     *   calling send()
     */
    int send(const void *buffer, int bufferLen);// gui du lieu
     ...
     */
    int recv(void *buffer, int bufferLen);// HAM NHAN DU LIEU
    ...
    void close(); //Đóng socket hoàn toàn, giải phóng tài nguyên.
    bool isAlive();// Kiểm tra xem socket còn hoạt động hay đã bị đóng.
    /**
