...
public:
     ...
     */
    string getLocalAddress();// xem thong tin socket cuc bo
     ...
     */
    void setLocalPort(unsigned short localPort);// gan dia chi va cong cho socket
    ...
    static unsigned short resolveService(const string &service,
                                         const string &protocol = "tcp");// chuyen ten dich vu thanh cong


    /**
     *   If WinSock, unload and clean up the WinSock DLLs;
     */
    static void cleanUp();// giai phong thu vien winSock khi chay

    /**
     *   If WinSock, load and start up the WinSock DLLs;
     */
    static void startUp();// khoi tao
